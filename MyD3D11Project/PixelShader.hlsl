...
float4 main(VS_OUTPUT input) : SV_Target
{
	...
	//vLightDir = vLightDir - float3(input.posForLight.x, input.posForLight.y, input.posForLight.z);
	//vLightDir = vLightDir - float3(input.pos.x, input.pos.y, input.pos.z);
	vLightDir = float3(input.pos.x, input.pos.y, input.pos.z) - vLightDir;
	...
	float lightDotNorm = dot(vLightDir, norm);

	if (lightDotNorm > 0.0f)
	{
		float projLight = saturate(lightDotNorm);
	...
	//specularLight = 2 * (lightDotNorm) * (vLightDir - norm) * vCol;
	//return lightColor;
